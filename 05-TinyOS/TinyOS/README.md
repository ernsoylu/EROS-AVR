# TinyOS — an OSEK BCC1 kernel for the Arduino Nano
...
Pure C99, no Arduino framework, no heap, no runtime object creation.
...
```
kernel/               app-agnostic kernel (reused by 07-ComprehensiveDemo)
...
config.h / config.c   the application's static configuration ("OIL file"):
                      tasks, priorities, alarms, resources, pool geometry,
                      hooks, aliveness mask — const tables in PROGMEM
main.c                demo application (tasks + hooks)
Makefile              mandated warning-free flags, .map file, size/budget
                      targets, avrdude flash target (old-bootloader baud)
```

The kernel never contains a `config.h`; each application provides its
own and compiles the kernel sources against it (`-I.` first) — the OSEK
"one kernel, per-app static config" model. Example 07 reuses this kernel
unchanged with a completely different task set.
...
- **IPC** — single-slot mailbox transporting handles of an O(1)
  ...
  allocation mask gives O(1) double-free detection). Empty/full are
  normal polling outcomes and do not raise `ErrorHook`.
...
`ChainTask`, no `AppModeType`, reduced `StatusType` set) are documented
in the same header and in `tiny_os.h`.
