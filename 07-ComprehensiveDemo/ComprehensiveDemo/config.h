 ...
 *
 * This application re-uses the TinyOS kernel from 05-TinyOS unchanged -
 * only this configuration and the task set differ, demonstrating the
 * OSEK model of one app-agnostic kernel + per-application static config.
 *
 * Integration map (what each earlier example contributes):
 *   01-Setup          -> TASK_STATUS heartbeat LED on PB5 (D13)
 *   02-GPIO           -> TASK_BUTTON debounced input on PD2 (D2), pull-up
 *   03-SerialMonitor  -> TASK_CMD command parser over interrupt-driven
 *                        UART (ON / OFF / STAT), TASK_STATUS reporting
 *   04-ManualScheduler-> replaced wholesale by TinyOS alarms (10/50/
 *                        100/500 ms periodic tasks instead of ISR flags)
 *   06-PWM            -> TASK_RAMP breathing LED on OC1A/PB1 (D9),
 *                        Timer1 fast PWM (Timer2 belongs to the OS tick!)
 */
