/**
 * @file    uart.h
 * @brief   Interrupt-driven UART driver (evolution of 03-SerialMonitor).
 *
 * 03-SerialMonitor used a POLLED driver: uart_print() busy-waits on UDRE0
 * for every byte, blocking the CPU for ~1 ms per character at 9600 baud.
 * Inside an RTOS that would stall every other task, so this driver is
 * fully interrupt-driven and non-blocking:
 *
