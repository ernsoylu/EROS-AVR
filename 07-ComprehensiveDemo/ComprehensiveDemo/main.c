/**
 * @file    main.c
 * @brief   Comprehensive demo - every earlier example, integrated on the
 *          TinyOS OSEK BCC1 kernel from 05-TinyOS.
 *
 * Hardware (Arduino Nano):
 *   PB5 / D13 : heartbeat LED, toggled every 500 ms       (from 01-Setup)
 *   PD2 / D2  : push button to GND, internal pull-up      (from 02-GPIO)
 *   PD0/PD1   : UART 9600 8N1 serial monitor              (from 03-SerialMonitor)
 *   PB1 / D9  : PWM "breathing" LED, Timer1 @ 1 kHz       (from 06-PWM)
 *   Scheduling: TinyOS alarms - 10/50/100/500 ms          (replaces 04-ManualScheduler)
 *
 ...
 * in hardware (single atomic store). The earlier examples used
 * 'PIND |= (1 << PDx)' - a read-modify-write that toggles EVERY pin of
 * the port that currently reads high, which happens to work only while
 * one pin is involved. 'PINB = (1 << PB5)' is the correct form.
 */
...
/**
 * TASK_STATUS - 500 ms. Heartbeat (01-Setup, done right: atomic PINx
