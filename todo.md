...
- [x] **Contract phase** (increment 2): each SWC gets a per-SWC `Rte_<SWC>.h`
      application header (`Rte_Run_<SWC>` + a port summary) so it compiles/tests
      standalone before the full system is configured. Additive - the combined
      `Rte.h` is unchanged; `cli.write` emits one per model/hand-ASW SWC.
- [x] **Rate transitions** (increment 2): already generated by the ASW->ASW
      `port.source` mechanism, and it works **cross-rate** - on the non-preemptive
      run-to-completion kernel the RTE's latched-global copy IS the whole
